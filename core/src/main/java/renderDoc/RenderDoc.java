	...
	static {
		...
		if (osName.contains("win") || !osName.contains("mac")) {
			try {
				...
				if (initResult != 1) {
					Log.LOGGER.error("Could not connect to RenderDoc API, return code: {}", initResult);
				} else {
					...
					Log.LOGGER.info("Connected to RenderDoc API v" + major.getValue() + "." + minor.getValue() + "." + patch.getValue());
				}
			} catch (UnsatisfiedLinkError ignored) {
			}
		}
		Log.LOGGER.info("renderDoc available:" + (apiInstance == null ? "failed" : "success"));
		renderdoc = apiInstance;

		if (renderdoc != null) {
			setCaptureOption(CaptureOption.API_VALIDATION, true);
			setCaptureOption(CaptureOption.CAPTURE_ALL_CMD_LISTS, true);
			setCaptureOption(CaptureOption.HOOK_INTO_CHILDREN, true);
			setCaptureOption(CaptureOption.REF_ALL_RESOURCES, true);
			setCaptureOption(CaptureOption.VERIFY_BUFFER_ACCESS, true);
		}
	}
